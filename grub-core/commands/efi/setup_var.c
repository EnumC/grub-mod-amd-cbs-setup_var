...
/* mod by EnumC @ enumc.com adding AMDCBS and var listing */
#include <grub/term.h>
...
*/
#define INSYDE_SETUP_VAR              	((grub_efi_char16_t*)"A\0m\0d\0S\0e\0t\0u\0p\0\0\0")
...
"\n\n(c) 2009 by Bernhard Froemel <bfroemel@gmail.com>\n"
	...
	grub_printf(
"\n\nForked by EnumC <eric@enumc.com> 02242019\n\nPlease note that this variant modifies AmdSetup instead of Setup\n"
	);
}

	name[0] = 0x0;
	/* scan for Setup variable */
	grub_printf("Looking for AmdSetup variable...\n");
	do
	{
...
/* obtain current contents of Setup variable */
	if(argc >= 1 && argc < 3)
	{
		...
		grub_printf("successfully obtained \"AmdSetup\" variable from VSS (got %d (0x%x) bytes).\n", (int)setup_var_size, (int)setup_var_size);
